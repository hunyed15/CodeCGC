...
全局安装后，postinstall 自动释放 `/cgc` 和 `/cgc-init` 到 `~/.claude/skills/`，并注册 `codecgcmcp`。
在项目中执行 `/cgc-init` 后，会释放项目级 skills，例如 `/cgc-entry`、`/cgc-plan`、`/cgc-build`、`/cgc-review` 等。
...
| 命令 | 用途 |
|------|------|
| `cgc "<需求>"` | 自然语言创建 workflow |
| `cgc-init` | 初始化项目 |
| `cgc-entry` | 创建/恢复 workflow |
| `cgc-plan` | 写入 workflow steps |
| `cgc-build` | 执行 feature 步骤 |
| `cgc-fix` | 执行 issue 修复 |
| `cgc-test` | 执行测试步骤 |
| `cgc-review` | 审核步骤结果 |
| `cgc-status` | 查看 workflow 状态 |
| `cgc-history` | 查询 workflow 和 audit 历史 |
| `cgc-doctor` | 环境健康检查 |
| `cgc-route` | 判断路径归属 |
| `cgc-audit` | 审计 workflow 完整性 |
| `cgc-manual` | 手动标记 docs/orchestration 步骤完成 |
| `cgc-continue` | 续接执行 session |
| `cgc-mcp` | 启动 MCP 服务器 |
