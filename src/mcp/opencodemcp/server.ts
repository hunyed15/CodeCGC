...
import type { FrontendTaskOptions, OpenCodeOptions } from "../../shared/types.js";
...
const TOOLS: Tool[] = [
  {
    name: "opencode",
    description: "通用 OpenCode CLI 会话执行器",
    inputSchema: {
      type: "object",
      properties: {
        PROMPT: { type: "string", description: "发送给 OpenCode 的指令" },
        cd: { type: "string", description: "工作区根目录" },
        sandbox: { type: "boolean", default: false, description: "是否开启 OpenCode 沙箱" },
    ...
    inputSchema: {
      ...
      properties: {
        ...
        target_paths: {
          type: "array",
          items: { type: "string" },
          description: "OpenCode 允许触碰的前端路径",
        },
...
const server = new Server({ name: "opencodemcp", version: "0.1.0" }, { capabilities: { tools: {} } });
...
server.setRequestHandler(CallToolRequestSchema, async (request) => {
  ...
  try {
    if (name === "opencode") {
      ...
      const result = await runOpenCodeSession(opts);
      return {
        content: [
          {
            type: "text",
...
async function main() {
  ...
  console.error("opencodemcp MCP server running on stdio");
}
